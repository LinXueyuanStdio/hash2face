...
function postMessage(object, transferList) {
  // ignore transferList
...
function onmessage(event) {
  switch (event.data.type) {
    case "run":
      try {
        var data_offset = [
          Module._get_static_buffer(),
          Module._get_dynamic_buffer()
        ];
        for (var i = 0; i < event.data.inputs.length; i++) {
          var var_alloc = event.data.inputs[i];
          var data_buf = new Float32Array(
            Module.buffer,
            data_offset[var_alloc.space] +
              var_alloc.offset * Float32Array.BYTES_PER_ELEMENT,
            var_alloc.size
          );
        ...
        for (var i = 0; i < event.data.outputs.length; i++) {
          ...
          var data_buf_view = new Float32Array(
            Module.buffer,
            data_offset[var_alloc.space] +
              var_alloc.offset * Float32Array.BYTES_PER_ELEMENT,
            var_alloc.size
    ...
    case "weight":
      try {
        var weight_buf = new Float32Array(
          Module.buffer,
          Module._get_static_buffer(),
    ...
    case "set_dynamic_buffer":
      try {
        ...
        while (data_idx < data_to_set.length) {
          Module._set_placeholder_value(
            data_to_set[data_idx],
            data_to_set[data_idx + 1],
            data_to_set[data_idx + 2]
          );
          data_idx += 3;
        }
        postMessage(0);
...
function integrateWasmJS(Module) {
  ...
  var wasmTextFile = Module["wasmTextFile"] || "kernels_webassembly.wast";
  var wasmBinaryFile = Module["wasmBinaryFile"] || "kernels_webassembly.wasm";
  var asmjsCodeFile =
    Module["asmjsCodeFile"] || "kernels_webassembly.temp.asm.js";
  ...
  async function getBinaryPromise() {
    const wasmBinaryFile = process.cwd() + "/models/Bouvardia128_8bit/kernels_webassembly.wasm"
    const buf = await qfs.read(wasmBinaryFile, "b")
    return buf.buffer
...
var _get_dynamic_buffer = (Module["_get_dynamic_buffer"] = function() {
  return Module["asm"]["_get_dynamic_buffer"].apply(null, arguments);
});
var setThrew = (Module["setThrew"] = function() {
  return Module["asm"]["setThrew"].apply(null, arguments);
});
var _get_static_buffer = (Module["_get_static_buffer"] = function() {
  return Module["asm"]["_get_static_buffer"].apply(null, arguments);
});
...
var _allocate_dynamic_buffer = (Module[
  "_allocate_dynamic_buffer"
] = function() {
  return Module["asm"]["_allocate_dynamic_buffer"].apply(null, arguments);
});
...
var _set_placeholder_value = (Module["_set_placeholder_value"] = function() {
  return Module["asm"]["_set_placeholder_value"].apply(null, arguments);
});
